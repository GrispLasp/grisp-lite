-module(grisplite_generic_tasks_worker).
...
-record(state, {running_tasks = [],
                finished_tasks = [],
                restart_interval = grisplite_config:get(generic_tasks_restart_interval, ?MIN)}).
...
handle_call({find_and_start_task}, _From, State = #state{running_tasks=RTasks, finished_tasks=FinishedTasks}) ->
    ...
    TasksList = grisplite_generic_tasks_server:get_all_tasks(),
